...
function registerPlayerFromWeb(playerName) {
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(10000);
  } catch (e) {
  ...
  try {
    ...
    if (!pSheet) pSheet = ss.insertSheet("Players");

    const data = pSheet.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][1]).trim().toLowerCase() === cleanName.toLowerCase()) {
        return { success: false, message: "A player with this name is already registered." };
      }
    }

    const result = addPlayer(cleanName);
    if (!result || !result.success) {
      return { success: false, message: "Failed to register player." };
    }

    const updatedPlayers = getPlayers();
    const newPlayer = updatedPlayers.find(p => p.name.toLowerCase() === cleanName.toLowerCase());
    const playerNum = newPlayer ? newPlayer.id : "Registered";

    return { success: true, playerName: cleanName, playerNumber: playerNum };
  } catch (e) {
